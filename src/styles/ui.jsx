...
const BackendService = {
  ...
  generateZip: async (bundle) => {
    ...
    if (!bundle || !bundle.id) {
      throw new Error('Invalid bundle: missing bundle ID');
    }
    if (!bundle.cards || !Array.isArray(bundle.cards) || bundle.cards.length === 0) {
      throw new Error('Invalid bundle: no cards found');
    }
    if (!bundle.config) {
      bundle.config = { autoRefresh: false, refreshInterval: 30, apiMode: 'rest' };
    }
    ...
    try {
      ...
      // README
      zip.file('README.md', `# CDAH Intelligence Card Bundle

**Bundle ID**: ${bundle.id}
**Created**: ${bundle.createdAt ? new Date(bundle.createdAt).toLocaleString() : new Date().toLocaleString()}
**Cards Included**: ${bundle.cards.length}

## Cards
${bundle.cards.map(c => `- ${c.title || 'Untitled'} (${c.type || 'unknown'})`).join('\n')}

## Quick Start
1. Copy the card components from \`cards/\` into your React application
2. Import the card you need: \`import ForecastDecisionCard from './cards/ForecastDecisionCard'\`
3. Use the card with your data: \`<ForecastDecisionCard data={yourData} />\`
4. See \`examples/\` for complete integration examples

## Configuration
- Auto-refresh: ${bundle.config.autoRefresh ? 'Enabled (' + bundle.config.refreshInterval + 'min)' : 'Disabled'}
- API Mode: ${bundle.config.apiMode}
${bundle.config.webhookUrl ? '- Webhook: ' + bundle.config.webhookUrl : ''}

## Documentation
See \`docs/\` folder for complete API documentation and integration guides.

## Support
Email: support@cdah.global | Docs: https://cdah.global/docs
`);

    // VERSION
    zip.file('VERSION.txt', `CDAH Intelligence Cards v1.0.0\nBundle: ${bundle.id}\nGenerated: ${new Date().toISOString()}\n`);

    // Config JSON
    zip.file('config.json', JSON.stringify(bundle.config, null, 2));

    // Card components
    const cardsFolder = zip.folder('cards');
    
    // Add ForecastDecisionCard component
    cardsFolder.file('ForecastDecisionCard.jsx', `import React from 'react';

const ForecastDecisionCard = ({ data }) => {
  ...
  return (
    <div style={{ fontFamily: 'IBM Plex Sans, system-ui, sans-serif', background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: 24, maxWidth: 900 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>{title}</h2>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ background: riskBg[riskLevel], color: riskColors[riskLevel], padding: '6px 12px', borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
            {riskLevel} Risk
          </div>
          <div style={{ fontSize: 13, color: '#64748B' }}>Confidence: {confidence}%</div>
        </div>
      </div>

      {/* Forecast Chart Placeholder */}
      <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 20, marginBottom: 20, minHeight: 200 }}>
        <div style={{ fontSize: 13, color: '#64748B', marginBottom: 10 }}>8-Week Forecast Trend</div>
        <div style={{ fontSize: 12, color: '#94A3B8' }}>Chart: {forecastData.length} weeks of data available</div>
      </div>

      {/* Model Performance */}
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Model Performance</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          <div style={{ background: '#F8FAFC', padding: 12, borderRadius: 6 }}>
            <div style={{ fontSize: 11, color: '#64748B', marginBottom: 4 }}>Accuracy</div>
            <div style={{ fontSize: 18, fontWeight: 600 }}>{modelPerformance.accuracy}%</div>
          </div>
          <div style={{ background: '#F8FAFC', padding: 12, borderRadius: 6 }}>
            <div style={{ fontSize: 11, color: '#64748B', marginBottom: 4 }}>MAPE</div>
            <div style={{ fontSize: 18, fontWeight: 600 }}>{modelPerformance.mape}%</div>
          </div>
          <div style={{ background: '#F8FAFC', padding: 12, borderRadius: 6 }}>
            <div style={{ fontSize: 11, color: '#64748B', marginBottom: 4 }}>R² Score</div>
            <div style={{ fontSize: 18, fontWeight: 600 }}>{modelPerformance.r2}</div>
          </div>
        </div>
      </div>
      ...
      {/* Feature Importance */}
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Key Risk Factors</h3>
        {featureImportance.map((f, i) => (
          <div key={i} style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
              <span>{f.feature}</span>
              <span style={{ fontWeight: 600 }}>{(f.importance * 100).toFixed(0)}%</span>
            </div>
            <div style={{ background: '#E2E8F0', borderRadius: 4, height: 6, overflow: 'hidden' }}>
              <div style={{ background: '#0072BC', width: (f.importance * 100) + '%', height: '100%' }} />
            </div>
            <div style={{ fontSize: 11, color: '#64748B', marginTop: 2 }}>{f.trend}</div>
          </div>
        ))}
      </div>
      ...
      {/* Recommendations */}
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Recommended Actions</h3>
      ...
      {/* Provenance */}
      <div style={{ fontSize: 11, color: '#94A3B8', borderTop: '1px solid #E2E8F0', paddingTop: 12 }}>
...
export default ForecastDecisionCard;
`);

    // Add SupplyDecisionCard component
    cardsFolder.file('SupplyDecisionCard.jsx', `import React from 'react';

const SupplyDecisionCard = ({ data }) => {
  const { vaccineName, stock, stockoutRisk, facilities, alerts, recommendations } = data;
  ...
  return (
    <div style={{ fontFamily: 'IBM Plex Sans, system-ui, sans-serif', background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: 24, maxWidth: 900 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>{vaccineName} Supply Status</h2>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ background: stockBg[stock], color: stockColors[stock], padding: '6px 12px', borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
            {stock}
          </div>
          <div style={{ fontSize: 13, color: '#64748B' }}>Stockout Risk: {stockoutRisk}%</div>
        </div>
      </div>
      ...
      {alerts.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          {alerts.map((alert, i) => (
            <div key={i} style={{ background: alert.type === 'warning' ? '#FEF3C7' : '#FEE2E2', border: '1px solid ' + (alert.type === 'warning' ? '#D97706' : '#DC2626'), borderRadius: 8, padding: 12, marginBottom: 8, fontSize: 13 }}>
              <div style={{ fontWeight: 600, marginBottom: 4 }}>{alert.message}</div>
              <div style={{ fontSize: 12, color: '#64748B' }}>{alert.facility}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Facilities */}
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Facility Status</h3>
        {facilities.map((f, i) => (
          <div key={i} style={{ background: '#F8FAFC', borderRadius: 8, padding: 12, marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, color: f.status === 'ok' ? '#16A34A' : f.status === 'low' ? '#D97706' : '#DC2626', fontWeight: 600 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, fontSize: 12 }}>
              <div>
                ...
                <div style={{ fontWeight: 600 }}>{f.stock.toLocaleString()}</div>
              </div>
              <div>
                <div style={{ color: '#64748B', fontSize: 11 }}>Capacity</div>
                <div style={{ fontWeight: 600 }}>{f.capacity.toLocaleString()}</div>
              </div>
              <div>
                <div style={{ color: '#64748B', fontSize: 11 }}>Utilization</div>
                <div style={{ fontWeight: 600 }}>{f.utilization}%</div>
              </div>
            </div>
            <div style={{ marginTop: 8 }}>
              <div style={{ background: '#E2E8F0', borderRadius: 4, height: 6, overflow: 'hidden' }}>
                <div style={{ background: f.status === 'ok' ? '#16A34A' : f.status === 'low' ? '#D97706' : '#DC2626', width: f.utilization + '%', height: '100%' }} />
              </div>
            </div>
          </div>
        ))}
...
export default SupplyDecisionCard;
`);

    // Documentation
    const docsFolder = zip.folder('docs');
    docsFolder.file('API.md', `# Card API Documentation

## ForecastDecisionCard

### Props
- \`data\` (object, required): Forecast data object
...
\`\`\`javascript
{
  ...
  forecastData: Array<{ week: number, value: number, lower: number, upper: number }>,
  modelPerformance: { accuracy: number, mape: number, r2: number },
  featureImportance: Array<{ feature: string, importance: number, trend: string }>,
...
### Props
- \`data\` (object, required): Supply data object
...
\`\`\`javascript
{
  ...
  alerts: Array<{ type: 'warning' | 'critical', message: string, facility: string }>,
  recommendations: string[]
}
\`\`\`
`);

    docsFolder.file('INTEGRATION.md', `# Integration Guide

## Installation
1. Copy the card components from \`cards/\` folder
2. Install IBM Plex Sans font:
   \`\`\`html
...
function MyApp() {
  ...
  return <ForecastDecisionCard data={forecastData} />;
}
\`\`\`
...
function MyApp() {
  ...
  return <SupplyDecisionCard data={supplyData} />;
}
\`\`\`

## Data Fetching
Integrate with your backend API to fetch real-time data:
...
function MyApp() {
  ...
  useEffect(() => {
    fetch('/api/forecast/flu_russia')
      .then(res => res.json())
      .then(setData);
  }, []);

  if (!data) return <div>Loading...</div>;
  return <ForecastDecisionCard data={data} />;
}
\`\`\`
`);
    ...
    examplesFolder.file('basic-forecast.jsx', `import React from 'react';
import ForecastDecisionCard from '../cards/ForecastDecisionCard';

const exampleData = ${JSON.stringify(FORECAST_SCENARIOS.flu_russia, null, 2)};

export default function Example() {
  return <ForecastDecisionCard data={exampleData} />;
}
`);

    examplesFolder.file('basic-supply.jsx', `import React from 'react';
import SupplyDecisionCard from '../cards/SupplyDecisionCard';

const exampleData = ${JSON.stringify(SUPPLY_SCENARIOS.bcg, null, 2)};

export default function Example() {
  return <SupplyDecisionCard data={exampleData} />;
}
`);
      ...
      throw new Error('Failed to generate ZIP file: ' + (err.message || 'Unknown error'));
    }
  },
...
const HomePage = ({ setCurrentPage, user }) => {
  ...
  return (
    <div style={{ background: imacs.surfaceElevated }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            Transforming climate and health data into actionable intelligence to protect communities worldwide.
          </p>
          ...
          <div className="hero-cta-buttons" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            >
              View Documentation
              <Icon name="arrow_forward" size={18} color="currentColor" />
            </button>
          </div>
        </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 500, letterSpacing: '0.1em' }}>SCROLL</span>
          <Icon name="expand_more" size={20} />
        </div>
      </section>

      {/* ================================================================== */}
      {/* TYPES OF DECISIONS SUPPORTED SECTION */}
      {/* ================================================================== */}
      <section style={{ padding: '100px 24px', background: imacs.surface }}>
        <div style={{ width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', marginBottom: 16, color: imacs.accent, textTransform: 'uppercase' }}>Decision Support</div>
          <h2 style={{ fontSize: 36, fontWeight: 700, margin: '0 0 16px', color: imacs.onSurface, letterSpacing: '-0.02em' }}>Types of Decisions Supported</h2>
          <p style={{ fontSize: 15, color: '#42474E', lineHeight: 1.7, marginBottom: 32, maxWidth: 700 }}>
            The platform provides decision-ready intelligence across the full public health decision cycle.
          </p>

          {/* 4 Decision Type Cards wrapper */}
          <div style={{ background: '#FFFFFF', borderRadius: 4, border: '1px solid #C2C7CE', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.08)', padding: 32 }}>
            <div className="decisions-grid">
              {/* Surveillance & Early Warning */}
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(0, 114, 188, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="notifications_active" size={20} color="#0072BC" />
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Surveillance & Early Warning</div>
                </div>
                <p style={{ fontSize: 13, color: '#42474E', lineHeight: 1.6, margin: 0 }}>
                  Spot emerging risks early by monitoring trends, anomalies, and action thresholds across priority indicators.
                </p>
              </div>

              {/* Preparedness & Resource Planning */}
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(0, 137, 123, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="event_note" size={20} color="#00897B" />
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Preparedness & Resource Planning</div>
                </div>
                <p style={{ fontSize: 13, color: '#42474E', lineHeight: 1.6, margin: 0 }}>
                  Plan staffing, supplies, and surge capacity using forecasts and risk signals to reduce last-minute scrambling.
                </p>
              </div>

              {/* Response & Operational Action */}
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(245, 124, 0, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="bolt" size={20} color="#F57C00" />
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Response & Operational Action</div>
                </div>
                <p style={{ fontSize: 13, color: '#42474E', lineHeight: 1.6, margin: 0 }}>
                  Trigger timely field action with clear recommendations linked to signals, protocols, and local operational constraints.
                </p>
              </div>

              {/* Monitoring, Review & Accountability */}
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(123, 31, 162, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="monitoring" size={20} color="#7B1FA2" />
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Monitoring, Review & Accountability</div>
                </div>
                <p style={{ fontSize: 13, color: '#42474E', lineHeight: 1.6, margin: 0 }}>
                  Track whether actions are working over time and adjust decisions using consistent measures and evidence.
                </p>
              </div>
            </div>
          </div>
      ...
      <section style={{ padding: '100px 24px', background: imacs.surfaceVariant }}>
        <div style={{ width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', marginBottom: 16, color: imacs.primary, textTransform: 'uppercase' }}>Platform Architecture</div>
          <h2 style={{ fontSize: 36, fontWeight: 700, margin: '0 0 16px', color: imacs.onSurface, letterSpacing: '-0.02em' }}>IMACS System Components</h2>
          <p style={{ fontSize: 16, color: imacs.onSurfaceSecondary, lineHeight: 1.7, marginBottom: 40, maxWidth: 700 }}>
            The data platform provides programmatic access to IMACS infrastructure and intelligence services through standardized APIs.
          </p>

          {/* 3 Layer Cards wrapper */}
          <div style={{ background: imacs.surface, borderRadius: 12, border: `1px solid ${imacs.outline}`, boxShadow: `0 4px 12px ${imacs.shadow}`, padding: 'clamp(16px, 4vw, 40px)' }}>
            <div className="architecture-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
              {/* User / Product Layer - Primary Blue */}
              <div style={{ padding: 24, background: imacs.primaryContainer, borderRadius: 12, border: `2px solid ${imacs.primary}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                  <div style={{ width: 44, height: 44, background: imacs.primaryLighter, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="apps" size={22} color={imacs.primary} />
                  </div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: imacs.onSurface }}>User / Product Layer</div>
                    <div style={{ fontSize: 11, fontWeight: 600, color: imacs.primary, marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Active Layer</div>
                  </div>
                </div>
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  <li style={{ fontSize: 14, color: imacs.onSurfaceSecondary, padding: '8px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Icon name="chevron_right" size={16} color={imacs.primary} />
                    DaaS APIs (You are here)
                  </li>
                  <li style={{ fontSize: 14, color: imacs.onSurfaceSecondary, padding: '8px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Icon name="chevron_right" size={16} color={imacs.primary} />
                    Health Decision Cards
                  </li>
                  <li style={{ fontSize: 14, color: imacs.onSurfaceSecondary, padding: '8px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Icon name="chevron_right" size={16} color={imacs.primary} />
                    CI-EWS Dashboards
              ...
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(123, 31, 162, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="psychology" size={20} color="#7B1FA2" />
                  </div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>Intelligence Layer</div>
                  </div>
                </div>
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  <li style={{ fontSize: 13, color: '#42474E', padding: '6px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Icon name="chevron_right" size={14} color="#7B1FA2" />
                    ML Forecasting
                  </li>
                  <li style={{ fontSize: 13, color: '#42474E', padding: '6px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Icon name="chevron_right" size={14} color="#7B1FA2" />
              ...
              {/* Infrastructure Layer - Teal */}
              <div style={{ padding: 20, background: '#F4F6F8', borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, background: 'rgba(0, 121, 107, 0.082)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="storage" size={20} color="#00796B" />
                  </div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>Infrastructure Layer</div>
                  </div>
                </div>
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  <li style={{ fontSize: 13, color: '#42474E', padding: '6px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Icon name="chevron_right" size={14} color="#00796B" />
                    AWS S3 Data Lake
                  </li>
                  <li style={{ fontSize: 13, color: '#42474E', padding: '6px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Icon name="chevron_right" size={14} color="#00796B" />
      ...
      {/* Platform Scale and Coverage Section */}
      <section style={{ padding: '100px 24px', background: imacs.surface }}>
        <div style={{ width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ marginBottom: 56 }}>
            <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', marginBottom: 16, color: imacs.accent, textTransform: 'uppercase' }}>Platform Scale and Coverage</div>
            <h2 style={{ fontSize: 36, fontWeight: 700, margin: 0, color: imacs.onSurface, letterSpacing: '-0.02em' }}>Global Health Data Infrastructure</h2>
          </div>

          {/* 4 Metric Cards */}
          <div className="metrics-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            <div style={{ background: '#F4F6F8', borderRadius: 4, padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, background: 'rgba(0, 114, 188, 0.082)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon name="database" size={22} color="#0072BC" />
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 600, color: '#1A1C1E', marginBottom: 4 }}>5.2B+</div>
              <div style={{ fontSize: 13, color: '#42474E' }}>Total Records</div>
            </div>
            <div style={{ background: '#F4F6F8', borderRadius: 4, padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, background: 'rgba(0, 121, 107, 0.082)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon name="storage" size={22} color="#00796B" />
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 600, color: '#1A1C1E', marginBottom: 4 }}>40TB+</div>
              <div style={{ fontSize: 13, color: '#42474E' }}>Data Lake Size</div>
            </div>
            <div style={{ background: '#F4F6F8', borderRadius: 4, padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, background: 'rgba(245, 124, 0, 0.082)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon name="public" size={22} color="#F57C00" />
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 600, color: '#1A1C1E', marginBottom: 4 }}>194</div>
              <div style={{ fontSize: 13, color: '#42474E' }}>Countries</div>
            </div>
            <div style={{ background: '#F4F6F8', borderRadius: 4, padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, background: 'rgba(46, 125, 50, 0.082)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon name="bolt" size={22} color="#2E7D32" />
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 600, color: '#1A1C1E', marginBottom: 4 }}>&lt; 100ms</div>
              <div style={{ fontSize: 13, color: '#42474E' }}>Avg Latency</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '100px 24px', background: imacs.surfaceVariant }}>
        <div style={{ width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', marginBottom: 16, color: imacs.primary, textTransform: 'uppercase' }}>Target Users</div>
          <h2 style={{ fontSize: 36, fontWeight: 700, margin: '0 0 40px', color: imacs.onSurface, letterSpacing: '-0.02em' }}>Designed for Global Health Professionals</h2>

          {/* 4 User Cards */}
          <div className="user-cards-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
            {/* Public Health Officer - Blue */}
            <div className="anim d1" style={{ background: '#FFFFFF', borderRadius: 4, border: '1px solid #C2C7CE', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.08)', padding: 24 }}>
              <div style={{ width: 48, height: 48, background: 'rgba(21, 101, 192, 0.07)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Icon name="medical_services" size={24} color="#1565C0" />
              </div>
              <h4 style={{ fontSize: 17, fontWeight: 600, margin: '0 0 4px' }}>Public Health Officer</h4>
              <p style={{ fontSize: 13, color: '#42474E', margin: '0 0 20px' }}>National / District</p>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#42474E', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workflow</div>
              <ol style={{ listStyle: 'none', padding: 0 }}>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(21, 101, 192, 0.082)', color: '#1565C0', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>1</span>
                  <span style={{ lineHeight: 1.5 }}>Monitor surveillance dashboards</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(21, 101, 192, 0.082)', color: '#1565C0', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>2</span>
                  <span style={{ lineHeight: 1.5 }}>Generate epidemiological reports</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10 }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(21, 101, 192, 0.082)', color: '#1565C0', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>3</span>
                  <span style={{ lineHeight: 1.5 }}>Configure outbreak alerts</span>
                </li>
              </ol>
            </div>

            {/* Health Researcher - Purple */}
            <div className="anim d2" style={{ background: '#FFFFFF', borderRadius: 4, border: '1px solid #C2C7CE', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.08)', padding: 24 }}>
              <div style={{ width: 48, height: 48, background: 'rgba(123, 31, 162, 0.07)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Icon name="science" size={24} color="#7B1FA2" />
              </div>
              <h4 style={{ fontSize: 17, fontWeight: 600, margin: '0 0 4px' }}>Health Researcher</h4>
              <p style={{ fontSize: 13, color: '#42474E', margin: '0 0 20px' }}>Academic / NGO</p>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#42474E', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workflow</div>
              <ol style={{ listStyle: 'none', padding: 0 }}>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(123, 31, 162, 0.082)', color: '#7B1FA2', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>1</span>
                  <span style={{ lineHeight: 1.5 }}>Access historical datasets</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(123, 31, 162, 0.082)', color: '#7B1FA2', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>2</span>
                  <span style={{ lineHeight: 1.5 }}>Run analytical queries</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10 }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(123, 31, 162, 0.082)', color: '#7B1FA2', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>3</span>
                  <span style={{ lineHeight: 1.5 }}>Export for modeling</span>
                </li>
              </ol>
            </div>

            {/* Software Developer - Green */}
            <div className="anim d3" style={{ background: '#FFFFFF', borderRadius: 4, border: '1px solid #C2C7CE', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.08)', padding: 24 }}>
              <div style={{ width: 48, height: 48, background: 'rgba(46, 125, 50, 0.07)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Icon name="code" size={24} color="#2E7D32" />
              </div>
              <h4 style={{ fontSize: 17, fontWeight: 600, margin: '0 0 4px' }}>Software Developer</h4>
              <p style={{ fontSize: 13, color: '#42474E', margin: '0 0 20px' }}>Integration / Apps</p>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#42474E', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workflow</div>
              <ol style={{ listStyle: 'none', padding: 0 }}>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(46, 125, 50, 0.082)', color: '#2E7D32', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>1</span>
                  <span style={{ lineHeight: 1.5 }}>Integrate REST APIs</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(46, 125, 50, 0.082)', color: '#2E7D32', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>2</span>
                  <span style={{ lineHeight: 1.5 }}>Build custom dashboards</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10 }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(46, 125, 50, 0.082)', color: '#2E7D32', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>3</span>
                  <span style={{ lineHeight: 1.5 }}>Implement data pipelines</span>
                </li>
              </ol>
            </div>

            {/* Program Manager - Orange */}
            <div className="anim d4" style={{ background: '#FFFFFF', borderRadius: 4, border: '1px solid #C2C7CE', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.08)', padding: 24 }}>
              <div style={{ width: 48, height: 48, background: 'rgba(245, 124, 0, 0.07)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Icon name="analytics" size={24} color="#F57C00" />
              </div>
              <h4 style={{ fontSize: 17, fontWeight: 600, margin: '0 0 4px' }}>Program Manager</h4>
              <p style={{ fontSize: 13, color: '#42474E', margin: '0 0 20px' }}>Ministry / Organization</p>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#42474E', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workflow</div>
              <ol style={{ listStyle: 'none', padding: 0 }}>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(245, 124, 0, 0.082)', color: '#F57C00', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>1</span>
                  <span style={{ lineHeight: 1.5 }}>Track coverage gaps</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10, borderBottom: '1px solid #C2C7CE' }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(245, 124, 0, 0.082)', color: '#F57C00', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>2</span>
                  <span style={{ lineHeight: 1.5 }}>Monitor capacity</span>
                </li>
                <li style={{ fontSize: 13, color: '#42474E', padding: '8px 0', display: 'flex', gap: 10 }}>
                  <span style={{ width: 20, height: 20, background: 'rgba(245, 124, 0, 0.082)', color: '#F57C00', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>3</span>
                  <span style={{ lineHeight: 1.5 }}>Generate reports</span>
                </li>
              </ol>
            </div>
          </div>
      ...
      {/* Request Access CTA Section - only show when not logged in */}
      {!user && (
        <section style={{
          padding: '100px 24px',
          background: imacs.heroGradient,
          textAlign: 'center',
          position: 'relative',
        }}>
          <div style={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(135deg, rgba(0, 51, 102, 0.95) 0%, rgba(0, 77, 64, 0.92) 100%)',
          }} />
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 700, margin: '0 auto', padding: '0 24px' }}>
            <h2 style={{ fontSize: 40, fontWeight: 700, margin: '0 0 20px', color: '#FFFFFF', letterSpacing: '-0.02em' }}>Ready to Get Started?</h2>
            <p style={{ fontSize: 18, color: 'rgba(255, 255, 255, 0.85)', margin: '0 0 40px', lineHeight: 1.7 }}>
              Submit an access request today. Most public health organizations are approved within 2-3 business days.
            </p>
...
const CanvasPage = ({ setCurrentPage, user, canvasTiles, setCanvasTiles, onAddToBundle, openLoginModal }) => {
  ...
  // Export as PNG/JPEG - Using html2canvas to capture actual rendered cards
  const handleExportImage = async (format = 'png') => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.download = `canvas-${layoutName.replace(/\s+/g, '-').toLowerCase()}.${format}`;
        link.href = url;
        link.click();
        URL.revokeObjectURL(url);
      }, mimeType, 0.95);

      BackendService.logAudit('canvas_exported_image', `Exported canvas as ${format.toUpperCase()}`);
    } catch (err) {
...
function App() {
  ...
  return (
    <>
      {/* Navbar OUTSIDE the content div - required for fixed positioning */}
      <Navbar currentPage={currentPage} setCurrentPage={setCurrentPage} user={user} onLogout={handleLogout} />

      {/* Content with 64px padding for non-home pages */}
      <div style={{
